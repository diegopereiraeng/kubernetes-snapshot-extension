...
//            it.remove();
    ...
    public static String getMetricsPath(Map<String, String> config, String namespace, String node){
        if(!node.equals(ALL)){
            ...
            return String.format("%s%s%s%s%s", Utilities.getMetricsPath(config), METRIC_PATH_NODES, METRIC_SEPARATOR, node);  
        }
            ...
            return String.format("%s%s%s%s%s", Utilities.getMetricsPath(config), METRIC_PATH_NAMESPACES, METRIC_SEPARATOR, namespace);
        }
